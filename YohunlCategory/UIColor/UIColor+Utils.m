...
- (void)getRGBAComponents:(CGFloat[4])rgba
{
    ...
    //也可以使用方法  - (BOOL)getRed:(nullable CGFloat *)red green:(nullable CGFloat *)green blue:(nullable CGFloat *)blue alpha:(nullable CGFloat *)alpha
}
...
- (NSString *)hexString
{
    ...
    if (self.alpha < 1.0f)
    {
        ...
        return [NSString stringWithFormat:@"0x%.8x", self.RGBAValue];
    }
    else
    {
        //don't include alpha component
        return [NSString stringWithFormat:@"0x%.6x", self.RGBValue];
    }
}
